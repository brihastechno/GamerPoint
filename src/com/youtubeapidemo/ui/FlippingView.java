...
/**
 * A view which flips from one ImageView to another view using a 3D flip
 * animation.
 */
public class FlippingView extends FrameLayout {
	...
	public interface Listener {
		 ...
		 * 
		 * @param view
		 *            The FlippingView which has completed the flip.
		 */
	...
	/**
	 * Create a flipping view which performs a 3D flip animation from one view
	 * to another.
	 * 
	 * @param context
	 ...
	 */
	@SuppressLint("NewApi")
	public FlippingView(Context context, Listener listener, int width,
			int height) {
		super(context);
		...
		ObjectAnimator flipOutAnimator = ObjectAnimator.ofFloat(flipOutView,
				"rotationY", 0, 90);
		flipOutAnimator.setInterpolator(new AccelerateInterpolator());
		Animator flipInAnimator = ObjectAnimator.ofFloat(flipInView,
				"rotationY", -90, 0);
	...
	/**
	 * Listens to the end of the flip animation to signal to listeners that the
	 * flip is complete
	 */
