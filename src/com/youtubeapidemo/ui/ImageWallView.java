...
public class ImageWallView extends ViewGroup {
	...
	public ImageWallView(Context context, int imageWidth, int imageHeight,
			int interImagePadding) {
		super(context);
	...
	protected void onSizeChanged(int width, int height, int oldWidth,
		...
		numberOfRows += (height % (imageHeight + interImagePadding) == 0) ? 0
				: 1;

		if ((numberOfRows <= 0) || (numberOfColumns <= 0)) {
			throw new IllegalStateException(
					"Error creating an ImageWallView with "
							+ numberOfRows
							+ " rows and "
							+ numberOfColumns
		...
		for (int col = 0; col < numberOfColumns; col++) {
			for (int row = 0; row < numberOfRows; row++) {
				...
				if (images[elementIdx] == null) {
					...
					thumbnail.setLayoutParams(new LayoutParams(imageWidth,
							imageHeight));
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int height = getDefaultSize(displayMetrics.heightPixels,
				heightMeasureSpec);
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		for (int col = 0; col < numberOfColumns; col++) {
			for (int row = 0; row < numberOfRows; row++) {
				int x = (col - 1) * (imageWidth + interImagePadding)
						+ (row * (imageWidth / numberOfRows));
				int y = row * (imageHeight + interImagePadding);
				images[col * numberOfRows + row].layout(x, y, x + imageWidth, y
						+ imageHeight);
			}
		}
	}
	...
	public Pair<Integer, Integer> getNextLoadTarget() {
		...
		do {
			if (unInitializedImages.isEmpty()) {
				// Don't choose the first or last columns (since they are partly
				// hidden)
				nextElement = random.nextInt((numberOfColumns - 2)
						* numberOfRows)
						+ numberOfRows;
			} else {
				nextElement = unInitializedImages.get(random
						.nextInt(unInitializedImages.size()));
			}
