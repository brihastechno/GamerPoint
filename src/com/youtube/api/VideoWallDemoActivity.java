...
import com.youtubeapidemo.ui.FlippingView;
import com.youtubeapidemo.ui.ImageWallView;
...
/**
 * A demo application aimed at showing the capabilities of the YouTube Player
 * API. It shows a video wall of flipping YouTube thumbnails. Every 5 flips, one
 * of the thumbnails will be replaced with a playing YouTube video.
 */
public class VideoWallDemoActivity extends Activity implements
		FlippingView.Listener, YouTubePlayer.OnInitializedListener,
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		int maxAllowedNumberOfRows = (int) Math
				.floor((displayMetrics.heightPixels / displayMetrics.density)
						/ PLAYER_VIEW_MINIMUM_HEIGHT_DP);
		int numberOfRows = Math.min(maxAllowedNumberOfRows,
				MAX_NUMBER_OF_ROWS_WANTED);
		int interImagePaddingPx = (int) displayMetrics.density
				* INTER_IMAGE_PADDING_DP;
		int imageHeight = (displayMetrics.heightPixels / numberOfRows)
				- interImagePaddingPx;
		int imageWidth = (int) (imageHeight * THUMBNAIL_ASPECT_RATIO);

		imageWallView = new ImageWallView(this, imageWidth, imageHeight,
				interImagePaddingPx);
		...
		getFragmentManager().beginTransaction()
				.add(R.id.player_view, playerFragment).commit();
	...
	public void onInitializationFailure(YouTubeThumbnailView thumbnailView,
		...
		if (errorReason.isUserRecoverableError()) {
			if (errorDialog == null || !errorDialog.isShowing()) {
				errorDialog = errorReason.getErrorDialog(this,
						RECOVERY_DIALOG_REQUEST);
				errorDialog.show();
			}
		} else {
			String errorMessage = String.format(
					getString(R.string.error_thumbnail_view),
	...
	public void onInitializationSuccess(YouTubePlayer.Provider provider,
			YouTubePlayer player, boolean wasResumed) {
	...
	private void maybeStartDemo() {
		if (activityResumed && player != null && thumbnailLoader != null
			...
			thumbnailLoader.setPlaylist(PLAYLIST_ID); // loading the first
														// thumbnail will kick
														// off demo
	...
	protected void onResume() {
		...
		if (thumbnailLoader != null && player != null) {
			if (state.equals(State.UNINITIALIZED)) {
				...
				flipDelayHandler.sendEmptyMessageDelayed(0,
	...
	private void flipNext() {
		...
		if (state.equals(State.VIDEO_ENDED)) {
			...
			Pair<Integer, Integer> nextTarget = imageWallView
					.getNextLoadTarget();
		...
		flippingView.setFlipOutDrawable(imageWallView.getImageDrawable(
				flippingCol, flippingRow));
		imageWallView.setImageDrawable(flippingCol, flippingRow,
				thumbnailView.getDrawable());
	...
	public void onFlipped(FlippingView view) {
		...
		if (activityResumed) {
			...
			if (state.equals(State.VIDEO_BEING_FLIPPED_OUT)) {
				...
				playerView.setX(imageWallView.getXPosition(flippingCol,
						flippingRow));
				playerView.setY(imageWallView.getYPosition(flippingCol,
						flippingRow));
				imageWallView.hideImage(flippingCol, flippingRow);
